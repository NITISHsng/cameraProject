...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (allowedUpdates.email || allowedUpdates.operatorId) {
      ...
      if (allowedUpdates.email) duplicateQuery.email = allowedUpdates.email;
      if (allowedUpdates.operatorId) duplicateQuery.operatorId = allowedUpdates.operatorId;

      const duplicate = await collection.findOne(duplicateQuery);
      if (duplicate) {
        return NextResponse.json({ error: "An operator with this ID or email already exists" }, { status: 409 });
      }
    }
